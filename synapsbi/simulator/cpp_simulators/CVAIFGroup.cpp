...
CVAIFGroup::CVAIFGroup( NeuronID size, NodeDistributionMode distmode) : NeuronGroup(size,distmode)
{
	auryn::sys->register_spiking_group(this);
	if ( evolve_locally() ) init();
}

void CVAIFGroup::calculate_scale_constants()
{
	...
	scale_vtrace = exp(-auryn_timestep/tau_vtrace);
	scale_cexc = exp(-auryn_timestep/tau_cexc);
	scale_cinh = exp(-auryn_timestep/tau_cinh);
}

void CVAIFGroup::init()
{
	e_rest = -70e-3;
	e_rev = -80e-3;
	thr_rest = -50e-3;
	...
	vtrace = get_state_vector("vtrace");
	cexc = get_state_vector("cexc");
	cinh = get_state_vector("cinh");

	clear();
}

void CVAIFGroup::clear()
{
	clear_spikes();
	mem->set_all(e_rest);
	thr->set_all(0.);
	...
	vtrace->set_all(-70e-3);
	cexc->set_all(0.);
	cinh->set_all(0.);
}

void CVAIFGroup::random_adapt(AurynState mean, AurynState sigma)
{
...
void CVAIFGroup::free()
{
}

CVAIFGroup::~CVAIFGroup()
{
	if ( evolve_locally() ) free();
}

void CVAIFGroup::integrate_linear_nmda_synapses()
{
	...
	//update the voltage trace and codependent traces
	const AurynFloat mul_tau_cexc = auryn_timestep/tau_cexc;
	cexc->saxpy(mul_tau_cexc, t_exc);
	cexc->saxpy(-mul_tau_cexc, cexc);

	const AurynFloat mul_tau_cinh = auryn_timestep/tau_cinh;
	cinh->saxpy(mul_tau_cinh, t_inh);
	cinh->saxpy(-mul_tau_cinh, cinh);

	const AurynFloat mul_tau_vtrace = auryn_timestep/tau_vtrace;
	vtrace->saxpy(mul_tau_vtrace, mem);
	vtrace->saxpy(-mul_tau_vtrace, vtrace);
}
 ...
 */
void CVAIFGroup::integrate_membrane()
{
	// moving threshold
	thr->scale(scale_thr);
...
void CVAIFGroup::check_thresholds()
{
	mem->clip( e_rev, 0.0 );
...
void CVAIFGroup::set_tau_mem(AurynFloat taum)
{
	tau_mem = taum;
	calculate_scale_constants();
}

void CVAIFGroup::set_tau_ampa(AurynFloat taum)
{
	tau_ampa = taum;
	calculate_scale_constants();
}

AurynFloat CVAIFGroup::get_tau_ampa()
{
	return tau_ampa;
}

void CVAIFGroup::set_tau_gaba(AurynFloat taum)
{
	tau_gaba = taum;
	calculate_scale_constants();
}

AurynFloat CVAIFGroup::get_tau_gaba()
{
	return tau_gaba;
}

void CVAIFGroup::set_tau_nmda(AurynFloat taum)
{
	tau_nmda = taum;
	calculate_scale_constants();
}

AurynFloat CVAIFGroup::get_tau_nmda()
{
	return tau_nmda;
}

void CVAIFGroup::set_tau_adapt(AurynFloat taua)
{
	tau_adapt1 = taua;
	calculate_scale_constants();
}

AurynFloat CVAIFGroup::get_tau_adapt()
{
	return tau_adapt1;
}

void CVAIFGroup::set_ampa_nmda_ratio(AurynFloat ratio) 
{
 	A_ampa = ratio/(ratio+1.0);
	A_nmda = 1./(ratio+1.0);
}

void CVAIFGroup::set_tau_vtrace(AurynFloat taum)
{
	tau_vtrace = taum;
	calculate_scale_constants();
}

void CVAIFGroup::set_tau_cexc(AurynFloat taum)
{
	tau_cexc = taum;
	calculate_scale_constants();
}

void CVAIFGroup::set_tau_cinh(AurynFloat taum)
{
	tau_cinh = taum;
	calculate_scale_constants();
}